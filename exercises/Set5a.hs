...
data Person = GeneratePerson Int String
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = GeneratePerson 90 "Fred"

-- getName returns the name of the person
getName :: Person -> String
getName (GeneratePerson _ name) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (GeneratePerson age _) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (GeneratePerson age _) = GeneratePerson age name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (GeneratePerson _ name) = GeneratePerson age name
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix c1 c2) = [(v1 + v2) / 2 | (v1, v2) <- (zip (rgb c1) (rgb c2))]
rgb (Invert c) = [negate v + 1 | v <- (rgb c)]
...
fromList [] = Empty
fromList xs = Pair (fst $ head xs) (snd $ head xs) (fromList $ tail xs)
...
toNat :: Int -> Maybe Nat
toNat 0 = Just Zero
toNat n = if n < 0 then Nothing else Just $ PlusOne (maybe Zero id $ toNat (n-1))
...
fromBin' :: Int -> Bin -> Int
fromBin' i (End) = 0
fromBin' i (I b) = 2^i + fromBin' (i+1) b
fromBin' i (O b) = fromBin' (i+1) b
