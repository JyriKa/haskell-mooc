...
hSelectLines h nums = do
    xs <- hFetchLines h
    return $ map (\(_,e) -> e) $ filter (\(i,_) -> elem i nums) (zip [1..] xs)
...
fnd3 :: (Bool,String,a) -> Bool
fnd3 (b,_,_) = b

snd3 :: (Bool,String,a) -> String
snd3 (_,s,_) = s

tnd3 :: (Bool,String,a) -> a
tnd3 (_,_,i) = i
...
interact' f state = do
    ...
    let r = f (l,state)
        in do
            putStrLn (snd3 r)
            if fnd3 r then interact' f (tnd3 r) else return (tnd3 r)
