...
allEqual [] = True
allEqual xs = length xs == (length $ fst $ partition (xs !! 0 ==) xs)
...
rangeOf xs = foldr max (head xs) xs - foldr min (head xs) xs
...
--Challenge:
longestComp :: Ord a => [a] -> [a] -> [a]
longestComp a b
    | length a > length b   = a
    | length a < length b   = b
    | otherwise             = if (head a) > (head b) 
                              then b 
                              else a

longest :: Ord a => [[a]] -> [a]
longest xs = foldr (longestComp) (head xs) xs
...
incrementKey k xs = [inc | (x, y) <- xs, let inc = if x == k then (x, y+1) else (x, y)]
...
--Challenge 1 and 2:
freqsValue :: Num a => Maybe a -> Maybe a 
freqsValue (Just x) = Just (x+1)
freqsValue Nothing = Just 1

freqs :: (Eq a, Ord a) => [a] -> Map.Map a Int
freqs xs = foldr (\k values -> 
    Map.alter (freqsValue) k values) (Map.fromList []) xs
...
transferValue :: String -> Int -> Map.Map String Int -> Map.Map String Int
transferValue name amount bank =
  case Map.lookup name bank of
    Nothing  -> bank
    Just sum -> Map.insert name (sum+amount) bank

transfer :: String -> String -> Int -> Map.Map String Int -> Map.Map String Int
transfer from to amount bank = 
    if amount > 0 && Map.findWithDefault (0-amount) from bank >= amount && Map.member to bank
    then transferValue to amount (transferValue from (-amount) bank)
    else bank
...
maxIndex arr = fst $ sortBy (comparing snd) (assocs arr) !! (length arr - 1)
