...
powers :: Int -> Int -> [Int]
powers k max = reverse $ powersHelper k 0 max []
powersHelper :: Int -> Int -> Int -> [Int] -> [Int]
powersHelper k p max numbers = if k^p > max 
                                then numbers 
                                else powersHelper k (p+1) max (k^p:numbers)
...
whileRight f x = case f x of Right x -> whileRight f x
                             Left x -> x
...
multiComposeHelper [] f = f
multiComposeHelper fs f = multiComposeHelper (tail fs) (f . head fs)

multiCompose [] x = x
multiCompose fs x =  multiComposeHelper (tail fs) (head fs) x
...
--Challenge:
--multiApp :: ([b] -> c) -> [a -> b] -> a -> c
multiApp f gs x = f $ map execute gs 
    where execute fun = fun x
...
interpreter commands = exec commands 0 0
    where exec commands x y
            | null commands               = []
            | head commands == "up"       = exec (tail commands) x (y+1)
            | head commands == "down"     = exec (tail commands) x (y-1)
            | head commands == "left"     = exec (tail commands) (x-1) y
            | head commands == "right"    = exec (tail commands) (x+1) y
            | head commands == "printY"   = show y:(exec (tail commands) x y)
            | head commands == "printX"   = show x:(exec (tail commands) x y)
            | head commands == "quit"     = []
            | not (null commands)         = (exec (tail commands) x y)
